        ...
        <div class="register-container">
          ...
          <div class="nickname-panel">
            ...
            <input
              ...
              @blur="changePlaceholder($event)"
              :disabled="registering"
              @input="debonceLengh($event,nickLength)"
            />
          </div>
          <span class="contectInfo" id="nickContect">{{ nickLength }}</span>
          <!-- 账户 -->
          <div class="account-panel">
            ...
            <input
              ...
              :disabled="registering"
              @input="debonceLengh($event,accountLength)"
              
            />
          </div>
          <span class="contectInfo" name="acountContect" 
          :style="registerData.account.length < 4 || registerData.account.length > 16 ? {visibility: 'visible'} 
          : {visibility:'hidden'}">{{ accountLength }}</span>
          ...
          <div class="password-panel">
            ...
            id="passwordContect"
              ...
              :disabled="registering"
              @input="debonceLengh($event,`${passwordLength}`)"
          ...
          <span class="contectInfo" name="passwordContect">{{ passwordLength }}</span>
          ...
          <span class="contectInfo" name="confirmContect"></span>
...
let accountLength = ref('18长度的字符')
let nickLength = ref('')
let passwordLength = ref('')

let checkLength= (e,prompt) => {
  let length = e.target.value
  console.log('aaa',prompt);
  if (length < 4 || length > 16) {
    // errPrompt.innerHTML = '请输入6-18长度的字符'
    prompt = '请输入6-18长度的字符'
  } else {
    // errPrompt.innerHTML = ''
    prompt = ''
...
let timer = null
function debonceLengh(e,prompt) {
  console.log('进入了de',prompt);
  if (timer) {
    clearTimeout(timer)
  }
  timer = setTimeout(() => {
    checkLength(e,prompt)
  }, 300)
}
